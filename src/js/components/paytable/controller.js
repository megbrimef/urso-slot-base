class ComponentsPaytableController extends Urso.Core.Components.StateDriven.Controller {
    ...
    _calculateBasePayout() {
        ...
        payouts.forEach((payoutData) => {
            const { payout, symbols } = payoutData;
    ...
    _updatePaytableData(currentPayout) {
        const classParts = Object.values(currentPayout);
        classParts.forEach((classPart) => {
    ...
    _recalculatePaytable() {
        ...
        Object.keys(this._basePayout)
            .forEach((key) => { currentPayout[key] = this._basePayout[key] * bet; });

        this._updatePaytableData(currentPayout);
    }

    _setWindowVisibility(isVisible) {
        const paytable = Urso.findOne('.paytable');
        this._addRemoveActiveClass(paytable, isVisible);
    }

    _addRemoveActiveClass(obj, isActive) {
        if (isActive) {
            obj.addClass('active');
        } else {
    ...
    _setPageVisibility(newIndex) {
        ...
        pages.forEach((page) => {
            const isVisible = page._originalModel.name === nameToSetVisible;
            this._addRemoveActiveClass(page, isVisible);
        });
    }
