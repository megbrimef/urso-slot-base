class ComponentsMegaWinController extends Urso.Core.Components.Base.Controller {
    ...
    _animationHandler() {
        ...
        const firstStageSlotWin = slotMachineDataFirstStage.slotWin;
        const totalWin = firstStageSlotWin.totalWin;
        const bet = Urso.localData.get('bets.value');

        if (!firstStageSlotWin.lineWinAmounts.length || totalWin < bet * 10) {
            return
        }
        ...
        gsap.to(this.megaWin, {
            ...
            onComplete: () => {
                gsap.to(this.megaWin, { scaleX: 0, scaleY: 0, alpha: 0, delay: 1.5 })
            }
        });
    }
